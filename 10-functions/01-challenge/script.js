...
A poll has a question, an array of options from which people can choose, and an array with the number of replies for each option. This data is stored in 
...
let poll = {
  registerNewAnswer() {
    let answer = prompt(`What is ypour favourite programming language?
          0: JavaScript 
          1: Python
          2: Rust
          3: C++
    ...
    if (answer == 0 || answer == 1 || answer == 2 || answer == 3) {
      ...
      answers[answer] += 1;
      console.log(answers);
    } else console.log(`its not a valid input`);

    this.displayResults(answers);
  },
  displayResults(type) {
    console.log(`Poll's results are ${type}`);
  },
};
