...
let dogToHumanAge = [];
function calAverageHumanAge(dogsAges) {
  for (const age of dogsAges) {
    age <= 2 ? dogToHumanAge.push(2 * age) : dogToHumanAge.push(16 + age * 4);
  }
}
calAverageHumanAge(data2);

dogToHumanAge = dogToHumanAge.filter(function (value) {
  return value >= 18;
});

let total = dogToHumanAge.reduce((acc, value) => acc + value);

console.log(total);

const average = total / dogToHumanAge.length;
